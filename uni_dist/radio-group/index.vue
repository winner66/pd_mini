...
<wux-cell-group id="wux-cell-group" :wux-class="prefixCls" :prefixCls="cellGroupPrefixCls" :title="title" :label="label">
    <block v-for="(option, index) in common.getOptions(options)" :key="index" v-if="options.length > 0">
        <wux-radio :prefixCls="option.prefixCls || 'wux-radio'" :cellPrefixCls="option.cellPrefixCls || 'wux-cell'" :selectablePrefixCls="option.selectablePrefixCls || 'wux-selectable'" :thumb="option.thumb" :title="option.title" :label="option.label" :value="option.value" :checked="inputValue === option.value" :disabled="option.disabled" :color="option.color || 'balanced'" :data-index="index" @change="onRadioChange"></wux-radio>
    </block>
    <block v-if="options.length === 0">
        <slot></slot>
    </block>
</wux-cell-group>
</template>

<script module="common" lang="wxs" src="./common.wxs"></script>
...
baseComponent({
  ...
  methods: {
    ...
    changeValue(value = this.data.inputValue, options = this.data.options) {
      ...
      const keys = showOptions.length > 0 ? showOptions : elements ? elements.map(element => element.data) : []; // Elements should be updated when not using the options
