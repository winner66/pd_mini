<template>
<view :class="'wux-class ' + classes.wrap" v-if="values.length">
    <block v-for="(item, index) in values" :key="index">
    	<view :class="classes.item + ' ' + ( activeKey === index ? prefixCls + '__item--current' : '' )" :data-index="index" @tap="onTap">{{ item }}</view>
    </block>
</view>
</template>
